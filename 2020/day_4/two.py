...
trues = 0
for line in re.sub(r'\n(\S)', r' \1', open('input').read()).splitlines():
    doc = dict(v.split(':') for v in line.split())
    trues += all(f in doc for f in fields) and all(k == 'cid' or fields[k](v) for k, v in doc.items())
print(trues)
